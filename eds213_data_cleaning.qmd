...
```{r}
snowsurvey_csv[snowsurvey_csv=="."] = NA
...
- Since the df is reassigning into itself using this method, I do not need to use '<-' to assign it to 'snowsurvey_fixed' each time
...
- We are going to replace <1 with a 0, as the rest of the area columns add up to 100% without it. We have to use ""to make zero a character string for now, until we convert all the column to numeric
...
- By using 'as.numeric', we transform the cover columns to numeric types, and NA's are coerced to NA values (missing).
...
- There ended up being many values over 100, so we had to replace them all using the same method we used earlier
...
species_presence <- species_long %>%
  mutate(species_presence = ifelse(species_counts == "0", 0, 1))
...
species_presence <- species_presence %>%
  ...
  select(-c(Num_observers, All_obs_reported, Observer_hours, species_counts))
